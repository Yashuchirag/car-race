...
**Last completed:** Ardennes mountains and forest (section 3, WIP): mountains with rock on
their steep ground, conifer forest and boulders. 115 fps on High. Royal Park's countryside
you approved.

**Next action:** Your look at the Ardennes. Then Desert Park desert, Ise Bay coast,
Northants city and the Airfield night neon city.
...
| Scenery per circuit | WIP | 2026-09-24. Royal Park's countryside is built and you approved its look on 2026-09-24. Ardennes mountains and forest built 2026-09-24 and waiting on your look: `GroundBuilder` raises ridged mountains (up to about 390 m) from 300 m past the circuit and paints rock (ambientCG Rock030) where the ground is steeper than 24 degrees, which also turns the banks where the road runs through a dip into rock cuttings; `SceneryBuilder` plants a theme's `Planting`, for the Ardennes a denser conifer forest (half the land) with boulders, keeping trees off rock. 93 structures, 36,290 trees, rocks and bushes; 114.6 fps average, 1% low 66.5. Rock paint has to go on after the terrain asset exists, or it is silently not saved. `Editor/SceneryBuilder.cs` places by distance beyond the barriers (a chamfer distance grid) and keeps a taken map, so nothing sits within 3 m of a barrier and nothing needs a collider. Structures (Kenney Racing Kit, 74 on Monza): pit garages and a covered stand either side of the start, open stands with tents on the outside of the four tightest corners, a banner tower where braking starts for each corner, angled billboards on the straights. Trees (Kenney Nature Kit, 22,886 with bushes): woods on 30% of the land from two octaves of noise, bushes on their edges, single trees in the fields, thinning from 350 m to 700 m beyond the barriers, and only the simplest models past 250 m. `Editor/KenneyModels.cs` remaps each model's materials onto one shared, instanced material per name, puts back colours the importer brightened, and turns the leaves and bark from Kenney's teal and orange to natural greens and browns. Trees live in the terrain data; `Scripts/Game/InstancedTrees.cs` draws them in instanced batches (the terrain's own path drew them one call each: 44 fps), by model and 500 m of land, skipping batches out of view and out of shadow range, none past 1500 m. Benchmark on Monza, High, 1080p: 134.7 fps average, 1% low 74.7, worst frame 17.6 ms (210 before scenery). It is geometry bound, not pixel bound: Low quality and 960x540 changed little. |
