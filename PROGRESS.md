...
| Minimap | WIP | 2026-09-24, your request: the whole circuit in the bottom left, north up, the start line marked, a dot per car (you white and larger, the AI in their colours). `MiniMap` draws the outline once into a texture from `TrackPath.centre`. Compiles against the real Unity DLLs; the same drawing reproduced in Python matches the pipeline's own Monza plot, not mirrored. Waiting on your drive. |
