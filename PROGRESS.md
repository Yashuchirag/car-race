...
**Last completed:** Car design (section 3, WIP): the boxes are now low-poly GT coupes.

**Next action:** Your look at the new cars. The Ise Bay AI crash at 130R you will look at
later (section 6).
...
| Car design | WIP | 2026-09-24, your request: the cars were a box on four cylinders. `Editor/CarModel.cs` builds a low-poly GT coupe to the car's own dimensions (wheelbase 2.65 m, track 1.6 m, tyres 0.34 m, the arches placed from the weight split): a body lofted through cross-sections, rear bumper to nose, with arches over the tyres, a fastback rear window, roof and raked windscreen; tinted glass, black underbody, splitter, diffuser lip, a rear wing on struts, mirrors, glowing head and tail lights; each wheel a tyre and a rim with five spokes, so the wheels are seen to turn. Paint only on "Body", so the colour picker and the AI colours recolour nothing else; the loops that used to paint every other part tyre black now touch the body only. Paint smoothness 0.2 to 0.62, which also lets the black car show at night. Meshes saved once as `Assets/Cars/CarBody.asset` and `CarDetails.asset`, shared by every car. Checked in lobby and race screenshots, red, white, and black at night. Royal Park 112 to 115 fps (warm; 122 to 135 before), one 0.9 s stall on the first run after the build, not repeated in two more. Waiting on your look. |
